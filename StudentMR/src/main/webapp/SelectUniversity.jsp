    ...
    <body>
        ...
        <div class="container w-75 m-auto shadow p-3 mb-5 bg-white rounded">
            <div class="alert alert-light text-secondary w-50 m-auto " role="alert">
                Select <text class="text-primary">University</text> From below Dropdown
            </div>
            <br>
            <select id="universityname" class="form-control form-control-sm w-50 m-auto" onchange="navigateToUni()" >
                
                <%
                    Retriever rv = new Retriever();
                    Set<String> stu = rv.getUniversityList();
                    
                    for (String uni : stu) {
                        out.write("<option>" + uni + "</option>");
                    }
                    out.write("<option>__ALL__</option>");
                %>
            </select><!-- comment --></div>
    </body>
    <script>
        function navigateToUni()
        {
            ...
            window.location.replace("/StudentMR/DisplayReport.jsp?uname="+uname.value);
        }
    </script>
</html>
